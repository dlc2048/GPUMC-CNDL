    ...
    def _scattering(self, verbose):
        ...
        for i in tqdm(range(len(energy_mean))):
            ...
            if energy_mean[i] < energy_thermal: # free gas
                ...
                # generate equiprob bin
                for j in range(label[-1][2] - 1, label[-1][2] + len(data[-1]) - 2):
                    ...
                    for mu in np.linspace(-1, 1, equi_steps):
                        ...
                        prob_last = prob
                        mu_last = mu
                    equiprob_data = np.append(equiprob_data, equiprob_seg, axis=0)

            else: # high energy scattering
                ...
                prob = ebin_target[1:] - ebin_target[:-1]
                ...
                for j in range(gmin, gmax+1):
                    eb_min = self.egn[j]
                    eb_max = self.egn[j+1]
                    mu_min = labMu(eb_min/energy_mean[i], A)
                    mu_max = labMu(eb_max/energy_mean[i], A)
                    if mu_min > mu_max:
                        _temp = mu_min
                        mu_min = mu_max
                        mu_max = _temp
                    ...
                    equiprob_seg = np.ones((1, nbin + 1), dtype=np.float64)
                    equi_probs_offset = np.linspace(labMuCumul(mu_min, A, 0),
                                                    labMuCumul(mu_max, A, 0),
                                                    nbin+1)
                    
                    for k in range(nbin + 1):
                        equiprob_seg[0,k] = fsolve(labMuCumul, mu_min, args=(A, equi_probs_offset[k]))
                    equiprob_data = np.append(equiprob_data, equiprob_seg, axis=0)

        # save the equiprob map
