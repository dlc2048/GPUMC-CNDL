...
class CNDL(GendfInterface):
    ...
    def _scattering(self, verbose):
        ...
        # integrate MT2 and MT221 energy transition matrix
        matrix_temp = np.empty(self._gendf.reactions[2].mf[6]._getTransMatrix().shape, dtype=np.float64)
        matrix_temp[:thermal_thres] = np.copy(self._gendf.reactions[221].mf[6]._getTransMatrix()[:thermal_thres])
        matrix_temp[thermal_thres:] = np.copy(self._gendf.reactions[2].mf[6]._getTransMatrix()[thermal_thres:])

        self.reactions[2].mf[6] = copy.deepcopy(self._gendf.reactions[2].mf[6])
        self.reactions[2].mf[6].setFromMatrix(matrix_temp)
    ...
    def genEquiProb(self, verbose=False):
        ...
        for mt in self.reactions:
            if mt == 2: # elastic scattering. It follows different equiprob cosine generating scheme.
                self.reactions[mt].mf[6].genEquiProbMap()
            else:
