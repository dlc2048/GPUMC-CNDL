...
class CNDL(GendfInterface):
    ...
    def _scattering(self, verbose):
        ...
        for i in tqdm(range(len(self.reactions[2].mf[6].target_tape))):
            ...
            for ene_index, j in enumerate(range(group_start, group_start + target_end - target_start)):
                ...
                if i < thermal_thres: # thermal scattering (free gas)
                    ...
                    equiprob_seg = ad.getEquiAngularBin(ene_target[i][ene_index], self.egn[j], self.egn[j+1], nbin)
